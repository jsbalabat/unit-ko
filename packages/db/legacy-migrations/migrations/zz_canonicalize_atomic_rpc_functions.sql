-- Canonicalize atomic RPC function definitions.
--
-- Why this exists:
-- The same RPC names were defined in multiple prior migrations, and final behavior
-- depended on migration execution order. This migration reapplies the latest intended
-- definitions so environments converge to one deterministic implementation.
--
-- These definitions assume the schema as of drop_legacy_denormalized_columns.sql:
-- - tenants no longer has pax / pax_details columns.
-- - billing_entries no longer has tenant_payments / tenant_rent_amounts /
--   tenant_other_charges columns.
