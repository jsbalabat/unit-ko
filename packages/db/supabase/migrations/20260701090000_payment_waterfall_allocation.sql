-- Waterfall payment allocation. Previously record_payment_atomic inserted the
-- full amount against a single invoice, so overpaying a period drove its derived
-- balance negative in the SOA. Now a rent payment settles the targeted invoice
-- first, then the lease's other unpaid invoices oldest-first (each capped at its
-- remaining balance), and any surplus is booked as an unallocated lease-level
-- credit — never a negative balance. Moving this server-side makes it atomic and
-- authoritative for every caller (the web no longer allocates client-side).
--
...
begin
  ...
  if v_payment_type in ('deposit', 'advance') then
    insert into public.payments (
      ...
      null, v_lease_id, v_tenant_id, v_payment_type, v_amount, v_paid_at, p_landlord_id, v_notes
    )
    returning id into v_first_payment_id;
    return jsonb_build_object('paymentId', v_first_payment_id, 'billingEntryId', null);
  end if;
  ...
  if v_ids is not null then
    for v_i in 1 .. array_length(v_ids, 1) loop
      ...
      insert into public.payments (
        ...
        v_ids[v_i], v_lease_id, v_tenant_id, v_payment_type, v_apply, v_paid_at, p_landlord_id, v_notes
      )
  ...
  if v_remaining > 0 then
    insert into public.payments (
      ...
      null, v_lease_id, v_tenant_id, v_payment_type, v_remaining, v_paid_at, p_landlord_id, v_notes
    )
    returning id into v_pid;
    if v_first_payment_id is null then v_first_payment_id := v_pid; end if;
  end if;

  return jsonb_build_object('paymentId', v_first_payment_id, 'billingEntryId', v_first_entry_id);
end;
$$;
