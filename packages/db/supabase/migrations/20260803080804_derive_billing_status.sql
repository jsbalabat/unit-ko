alter table "public"."billing_entries" drop constraint "billing_entries_status_code_fkey";
...
alter table "public"."billing_entries" drop column "status_code";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.billing_entry_status(p_gross numeric, p_paid numeric, p_balance numeric, p_due date)
 RETURNS text
 LANGUAGE sql
 STABLE
...
CREATE OR REPLACE FUNCTION public.archive_and_reset_property_atomic(p_landlord_id uuid, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.claim_tenant_reminder(p_landlord_id uuid, p_billing_entry_id uuid, p_channel text DEFAULT 'email'::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.create_property_atomic(p_landlord_id uuid, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.create_unhoused_tenant_atomic(p_landlord_id uuid, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.replace_landlord_payout_methods(p_landlord_id uuid, p_payload jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.update_billing_entry_atomic(p_landlord_id uuid, p_entry_id uuid, p_payload jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.update_property_atomic(p_landlord_id uuid, p_property_id uuid, p_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
...
-- db diff drops each view's ACL when it recreates the view and never re-emits
-- grants (see the note in schemas/85_grants.sql). Re-grant the API role read
-- access on the views this migration recreated, or service_role reads 42501.
